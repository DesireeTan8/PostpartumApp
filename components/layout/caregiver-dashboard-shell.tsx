...
/** Caregiver dashboard frame: sidebar, top `Header`, bottom nav on mobile. Lives with other role shells under `components/layout/`. */
export function CaregiverDashboardShell({ children }: { children: React.ReactNode }) {
    const pathname = usePathname();
    const title = titleFromPath(pathname);

    const profileSlot = useMemo(
        () => (
            <Link href="/dashboard/caregiver/profile" className={HEADER_ICON_LINK} aria-label="Profile">
        ...
        <div className="min-h-dvh bg-[#f8f9fa] text-ink lg:flex">
            ...
            <div className="flex min-h-dvh flex-1 flex-col lg:ml-[260px]">
                <Header
                    ...
                    searchAriaLabel="Search caregiver app"
                    searchInputName="caregiver-app-search"
                />
